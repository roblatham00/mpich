...
#define PAIRTYPE_CONTENTS(mt1_,ut1_,mt2_,ut2_)                          \
    {                                                                   \
        ...
        disps[0] = 0;                                                   \
        disps[1] = DAME_VOID_PTR_CAST_TO_OFFSET ((char *) &foo.b - (char *) &foo.a); \
        types[0] = mt1_;                                                \
        types[1] = mt2_;                                                \
    }

/*@
  Dame_create_pairtype - create dataloop for a pairtype
  ...
  use with Dame_create_struct and then calls that function.
...
int MPIR_Dame_create_pairtype(MPI_Datatype type, DAME_Dame ** dl, MPI_Aint * size, int *depth)
{
    ...
    DAME_Assert(type == MPI_FLOAT_INT || type == MPI_DOUBLE_INT ||
    ...
    return MPIR_Dame_create_struct(2, blocks, disps, type, types, dl, size, depth);
}
