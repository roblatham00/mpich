...
#undef FUNCNAME
#define FUNCNAME MPIR_Type_convert_darray
#undef FCNAME
                             ...
                             int order, MPI_Datatype oldtype, MPI_Datatype * newtype)
{
    ...
    MPI_Aint *st_offsets, orig_lb, orig_extent, disps[3];

    MPIR_Type_get_extent_impl(oldtype, &orig_lb, &orig_extent);

    /* calculate position in Cartesian grid as MPI would (row-major
     * ordering) */
    coords = (int *) DAME_Malloc(ndims * sizeof(int));
    procs = size;
    tmp_rank = rank;
    for (i = 0; i < ndims; i++) {
    ...
    st_offsets = (MPI_Aint *) DAME_Malloc(ndims * sizeof(MPI_Aint));
    type_old = oldtype;
