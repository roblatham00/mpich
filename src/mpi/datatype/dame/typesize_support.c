...
static void DAME_Type_calc_footprint_struct(MPI_Datatype type,
                                            int combiner,
                                            int *ints,
                                            MPI_Aint * aints,
                                            MPI_Datatype * types, DAME_Type_footprint * tfp);
...
void MPIR_Type_calc_footprint(MPI_Datatype type, DAME_Type_footprint * tfp)
{
    ...
    DAME_Offset size = 0, lb = 0, ub = 0, true_lb = 0, true_ub = 0;
    DAME_Offset extent = 0, alignsz;
    ...
    DAME_Offset i, min_lb, max_ub, ntypes, tmp_lb, tmp_ub;
    ...
    if (combiner == MPI_COMBINER_NAMED) {
        int mpisize;
        MPI_Aint mpilb, mpiextent;

        MPIR_Type_size_impl(type, &mpisize);
        MPIR_Type_get_extent_impl(type, &mpilb, &mpiextent);
        tfp->size = (DAME_Offset) mpisize;
        tfp->lb = 0;
        tfp->ub = (DAME_Offset) mpiextent;
        tfp->true_lb = 0;
        tfp->true_ub = (DAME_Offset) mpiextent;
        tfp->extent = (DAME_Offset) mpiextent;
    ...
    if (combiner != MPI_COMBINER_STRUCT && combiner != MPI_COMBINER_STRUCT_INTEGER) {
        DAME_Type_footprint cfp;

        MPIR_Type_calc_footprint(types[0], &cfp);
        size = cfp.size;
        lb = cfp.lb;
        ub = cfp.ub;
        true_lb = cfp.true_lb;
    ...
    switch (combiner) {
        ...
        case MPI_COMBINER_CONTIGUOUS:
            ...
            tfp->size = (DAME_Offset) ints[0] * size;
        ...
        case MPI_COMBINER_HVECTOR_INTEGER:
            if (combiner == MPI_COMBINER_VECTOR)
                stride = (DAME_Offset) ints[2] * extent;
            else if (combiner == MPI_COMBINER_HVECTOR)
                stride = aints[0];
            else        /* HVECTOR_INTEGER */
                stride = (DAME_Offset) ints[2];
            ...
            tfp->true_lb = tfp->lb + (true_lb - lb);
            tfp->true_ub = tfp->ub + (true_ub - ub);
            tfp->size = (DAME_Offset) ints[0] * (DAME_Offset) ints[1] * size;
            tfp->extent = tfp->ub - tfp->lb;
            break;
        case MPI_COMBINER_INDEXED_BLOCK:
            /* prime min_lb and max_ub */
            DLOOP_DATATYPE_BLOCK_LB_UB(ints[1] /* blklen */ ,
                                       (DAME_Offset) ints[2] * extent /* disp */ ,
                                       lb, ub, extent, min_lb, max_ub);

            for (i = 1; i < ints[0]; i++) {
                DLOOP_DATATYPE_BLOCK_LB_UB(ints[1] /* blklen */ ,
                                           (DAME_Offset) ints[i + 2] * extent /* disp */ ,
        ...
        case MPI_COMBINER_HINDEXED_BLOCK:
            /* prime min_lb and max_ub */
            DLOOP_DATATYPE_BLOCK_LB_UB(ints[1] /* blklen */ ,
                                       (DAME_Offset) ints[2] /* disp */ ,
                                       lb, ub, extent, min_lb, max_ub);

            for (i = 1; i < ints[0]; i++) {
                DLOOP_DATATYPE_BLOCK_LB_UB(ints[1] /* blklen */ ,
                                           (DAME_Offset) ints[i + 2] /* disp */ ,
                                           lb, ub, extent, tmp_lb, tmp_ub);
                if (tmp_lb < min_lb)
                    min_lb = tmp_lb;
                if (tmp_ub > max_ub)
        ...
        case MPI_COMBINER_HINDEXED:
            ...
            if (i == ints[0]) {
                ...
                if (combiner == MPI_COMBINER_INDEXED)
                    disp = (DAME_Offset) ints[ints[0] + i + 1] * extent;
                else if (combiner == MPI_COMBINER_HINDEXED_INTEGER)
                    disp = (DAME_Offset) ints[ints[0] + i + 1];
        ...
        case MPI_COMBINER_STRUCT_INTEGER:
            DAME_Assert(combiner != MPI_COMBINER_STRUCT_INTEGER);
            break;
        case MPI_COMBINER_STRUCT:
            /* sufficiently complicated to pull out into separate fn */
            DAME_Type_calc_footprint_struct(type, combiner, ints, aints, types, tfp);
            break;
        case MPI_COMBINER_SUBARRAY:
            ...
            MPIR_Type_convert_subarray(ndims, &ints[1] /* sizes */ ,
                                       ...
                                       types[0], &tmptype);
            MPIR_Type_calc_footprint(tmptype, tfp);
...
/*
  DAME_Type_calc_footprint_struct - calculate size, lb, ub, extent,
                                            ...
                                            MPI_Datatype * types, DAME_Type_footprint * tfp)
{
    ...
    DAME_Offset tmp_lb, tmp_ub, tmp_extent, tmp_true_lb, tmp_true_ub;
    DAME_Offset max_alignsz = 0, tmp_size = 0, min_lb = 0, max_ub = 0;
    DAME_Offset min_true_lb = 0, max_true_ub = 0;

    int nr_ints, nr_aints, nr_types, combiner;

    /* used to store parameters for constituent types */
    DAME_Type_footprint cfp;
    DAME_Offset size, lb, ub, true_lb, true_ub, extent, alignsz;
    ...
    for (; i < ints[0]; i++) {
        ...
        MPIR_Type_calc_footprint(types[i], &cfp);
        size = cfp.size;
        lb = cfp.lb;
        ub = cfp.ub;
        true_lb = cfp.true_lb;
        ...
        tmp_size += size * (DAME_Offset) ints[i + 1];
...
static int DLOOP_Structalign_integer_max()
{
    ...
    DAME_Assert(is_packed + is_two + is_four + is_eight == 1);
...
static int DLOOP_Structalign_float_max()
{
    ...
    DAME_Assert(is_packed + is_two + is_four + is_eight + is_sixteen == 1);
