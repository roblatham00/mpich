...
{
    BV_IOContig(fd, buf, count, datatype, file_ptr_type, offset, status, BV_READ, error_code);
}
...
{
    BV_IOContig(fd,
                (void *) buf, count, datatype, file_ptr_type, offset, status, BV_WRITE, error_code);
}
